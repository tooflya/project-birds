...
public class GameHelper implements
		...
		OnSignOutCompleteListener, OnClickListener {
	...
	public void setup(GameHelperListener listener, int clientsToUse) {
		// setSigningInMessage("1");
		// setUnknownErrorMessage("Unknown error happens");
	...
	void showProgressDialog(boolean signIn) {
		return;/*
				 * String message = signIn ? mSigningInMessage :
				 * mSigningOutMessage;
				 * 
				 * if (mProgressDialog == null) { if (getContext() == null)
				 * return; mProgressDialog = new ProgressDialog(getContext()); }
				 * 
	...
	public native static void nativeOnGooglePlusSignInSucceeded();
}
