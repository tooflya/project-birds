...
- (void)open:(int)target
{
    ...
    {
        case 0:
        if(![[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"twitter://user?screen_name=tooflya"]])
        {
            NSURL *url = [[NSURL alloc] initWithString:@"https://www.twitter.com/tooflya"];
            [[UIApplication sharedApplication] openURL:url];
        }
        break;
        case 1:
            if(![[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"fb://user?screen_name=tooflya"]])
            {
                NSURL *url = [[NSURL alloc] initWithString:@"https://www.facebook.com/tooflya"];
                [[UIApplication sharedApplication] openURL:url];
            }
        break;
        case 2:
            if(![[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"vk://vk.com/tooflya"]])
            {
                NSURL *url = [[NSURL alloc] initWithString:@"https://www.vk.com/tooflya"];
                [[UIApplication sharedApplication] openURL:url];
            }
        break;
        case 3:
            {
                NSString *reviewURL = [NSString stringWithFormat:@"itms-apps://ax.itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?type=Purple+Software&id=%d", 0]; // TODO: APP_ID
...
- (void)postAchievement:(const char*)idName percent:(NSNumber*)percentComplete
{
    ...
    if (![GKLocalPlayer localPlayer].isAuthenticated) {
        ...
        NSLog(@"[GameCenter] not authenticated, so achievement %s saved to device only.", idName);
        return;
    }
    ...
    [achievement reportAchievementWithCompletionHandler:^(NSError* error) {
        if (error) {
            ...
            NSLog(@"[GameCenter] postAchievement for %s is succeed!", idName);
        }
    }];
}
